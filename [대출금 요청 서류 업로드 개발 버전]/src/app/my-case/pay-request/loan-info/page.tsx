...
export default function My_PR_006M() {
  ...
  const { seller, buyer, refetch, execDt, adminReqStatCd } = usePayInfoData({ loanNo });
  ...
  console.log("adminReqStatCd", adminReqStatCd);

  // NOTE: [임시] 서류 제출 결과 코드: 서류 확인 중(01), 서류 승인(02), 서류 반려(03) / 승인번호 인증 시(falsy)
  // 서류 제출 승인 결과 UI 노출 조건: 서류 제출 결과 코드가 서류 확인 중(01) 또는 서류 반려(03) 상태
  // 대출금 지급 결과 UI 노출 조건: 서류 제출 결과 코드가 서류 승인(02) 또는 빈 데이터 상태(falsy)

  // 서류 확인 중(01) 또는 서류 반려(02)
  const isDocumentUI = (adminReqStatCd === "01" || adminReqStatCd === "02");
      ...
      <div className="flex flex-col justify-between grow w-full h-full">
        <div>
          ...
          <div className="w-full flex flex-col gap-y-3">
            ...
            {/* 서류 확인 중 */}
            {adminReqStatCd === "01" && <PayProceeding text="서류 확인 중" />}
