...
import { formatDate, formatNumber, toggleDimmedBottomNavigation } from "@utils";
...
export default function CaseAcceptPage() {
  ...
  // 00시 ~ 09시 사이인지 체크
  const isBeforeNine = hours >= 0 && hours < 9;
    ...
    onSuccess: (checkAddInfo: TObj) => {
      ...
      if (authInfo.isRept && trregPssbYn === "N") {
        if (
          ...
          toggleDimmedBottomNavigation(true);
          reptOpen();
          if (prevPath === "/acceptance/match/add-info/profile") {
              ...
              <ul className="flex flex-col">
                {caseData?.data?.map((data: TCaseData) => (
                  <li
                    ...
                    } ${((data.order === "3" || data.order === "5") && isBeforeNine) ? "hidden" : ""}`}
                  >
