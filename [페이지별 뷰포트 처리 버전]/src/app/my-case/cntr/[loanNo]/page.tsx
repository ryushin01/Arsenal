...
export default function MyCaseDetail({
  ...
  let B200CallCheck: string | null;

  if (typeof window !== "undefined") {
    prevPath = sessionStorage.getItem("prevPath");
    checkPath = sessionStorage.getItem("pathCheck");
    // NOTE: 2025.05.23 B200 로딩 속도 이슈 처리 건
    // 견적서/등기정보/지급정보/대출금요청/지급결과/상환금요청 페이지에서 상세로 진입시 B200 호출 N
    if (
      checkPath === "/my-case/estm/regist" ||
      checkPath === "/my-case/estm/info" ||
      checkPath === "/my-case/estm/info/list" ||
      checkPath === "/my-case/pay-info" ||
      checkPath === "/my-case/pay-request/loan-pay" ||
      checkPath === "/my-case/pay-request/loan-info" ||
      checkPath === "/my-case/rpycncl"
    ) {
      B200CallCheck = "N";
    } else {
      B200CallCheck = "Y";
  ...
  const { data, refetch, isFetching } = useQuery({
    ...
    queryFn: (): Promise<TDetailInfo> =>
      fetchApi({
        url: `${process.env.NEXT_PUBLIC_APP_WOORI_API_URL}/api/cntr/searchCntrDetail?loanNo=${params.loanNo}&b200SendYN=${B200CallCheck}`,
        method: "get",
      })
        .then((res) => res.json())
        .then((res) => {
      ...
      <Header
        ...
        backCallBack={() => {
          if (prevPath === `/my-case/estm/info`) {
            sessionStorage.removeItem("pathCheck");
            // @ts-ignore
            window.flutter_inappwebview.callHandler("flutterFunc", {
